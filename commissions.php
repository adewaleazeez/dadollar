        ...
        <script type="text/javascript">
            $(function() {
                ...
                $("#transactionlist").dialog({
                    autoOpen: true,
                    position:'center',
                    title: 'Commission Transactions List',
                    height: 500,
                    width: 1200,
                    modal: false,
                    buttons: {
				...
				if(characterCode==13 || characterCode==9){ // if enter ot tab
					if(idcode=="cardnoid"){ 
						...
						if(isNaN(document.getElementById(old_id).value.replace(/,/g, '')+"")){
							alert('The value you typed for Amount is not a number!!!');
...
//									document.getElementById("balance_aid"+id2[1]).value="";
			...
			createCookie('currentform', 'commission', false);
        </script>
    </head>
    <body>
				...
				<table style='font-size:14px;'>
					<tr>
						...
						<td>
							...
							&nbsp;&nbsp;
							<input type="button" style="display:inline" value=" Calculate Commissions " id="calcom" onclick="checkCalcomm();" onkeydown="checkKeyPressed(event,this.id);" />
						</td>
					</tr>
					<tr>
						<td colspan="6">
			...
			<div id="transtotal">
				<table style='font-size:14px;'>
					<tr>
						<td width='5.2%'>&nbsp;</td>
						<td width='10%'>
							<b>Total commissions:</b>&nbsp;<input type="text" id="totalamount" size="10" disabled style='text-align:right;' />
						</td>
					</tr>
				</table>
			</div>
