...
//alert(table);
    ...
    document.getElementById("showAlert").innerHTML = "<b>Please wait...</b><br><br>Fetching your data.";
    $('#showAlert').dialog('open');
    var url = "/dadollar/setupbackend.php?option=getAllRecs" + "&table=" + table;
//alert(url);
    AjaxFunctionSetup(url);
}
...
function AjaxFunctionSetupSMS(arg) {
	...
	$.ajax({
		...
		success: function stateChangedSetupSMS(resp) {
			if (resp.trim().match("listSMSTransaction")) {
        ...
        if (resp.match("showCurrentCardno")) {
            ...
            if (break_resp[1] === readCookie("currentCardno")) {
                ...
                if (duplicatecounter >= 100) {
                    ...
                    //$('#showAlert').dialog('open');
                    alert("Transaction processing completed!!!");
                    clearInterval(myVar);
                    $('#showAlert').dialog('close');
                }
            } else {
...
//$('#showPrompt').dialog('open');
        ...
        if (resp.match("myresponse")) {
            break_resp = resp.split("myresponse");
            alert(break_resp[1]);
        }
...
//$('#showPrompt').dialog('open');
            ...
            if (break_resp[0] === "customers") {
                ...
                allrecords += "<td>S/No</td><td>Card No</td><td>Names</td><td align='center'>Trans.&nbsp;Date</td><td align='right'>Credit</td><td align='right'>Debit</td><td align='right'>Balance</td><td>Trans.&nbsp;Type</td></tr>";
            ...
            var flg = 0;
            var break_row = "";
            var compare1 = "customers deposits withdrawals loandeposits ";
            var compare2 = "customers deposits withdrawals loandeposits ";
            var compare3 = "customers deposits withdrawals loandeposits ";
            var compare4 = "customers withdrawals loandeposits ";
            var compare5 = "customers withdrawals loandeposits ";
            var compare6 = "customers ";
            myCheckboxes = 0;
            var serialnovalue = "";
            var serialnoid = "";
            var checkboxid = "";
            ...
            if (break_resp[0] === "withdrawals") {
                ...
                rsp += "<tr style='font-weight:bold; color:#FFFFFF;background-color:#FF8080;'>";
                rsp += "<td colspan='4'  style='text-align:right;'>Total:</td><td  style='text-align:right;'>" + numberFormat(totaldeposit + "") + "</td><td  style='text-align:right;'>" + numberFormat(totalwithdrawals + "") + "</td><td colspan='2'>&nbsp;</td></tr>";
                rsp += "<tr style='font-weight:bold; color:#FFFFFF;background-color:#FF8080;'>";
                rsp += "<td colspan='6'  style='text-align:right;'>Closing Balance:</td><td  style='text-align:right;'><input type='text' readonly  disabled='true' value='" + numberFormat(bals[1] + "") + "' style='display:inline; text-align:right;' size='15' /></td><td>&nbsp;</td></tr>";
            }
...
//$('#showPrompt').dialog('open');
            ...
            if (break_resp[0] === "customers") {
                if (readCookie("names").match(null)) {
                    ...
                    if (break_resp[13] !== null && break_resp[13] !== "") {
                        var newdate = break_resp[13].substr(0, 10);
                        newdate = newdate.split("-");
                        break_resp[13] = newdate[2] + "/" + newdate[1] + "/" + newdate[0];
                        document.getElementById("datedisbursed").value = break_resp[13];
                        document.getElementById("loanamount").value = numberFormat(break_resp[14]);
                        document.getElementById("loaninterest").value = numberFormat(break_resp[15]);
                        var newdate = break_resp[16].substr(0, 10);
                        newdate = newdate.split("-");
                        break_resp[16] = newdate[2] + "/" + newdate[1] + "/" + newdate[0];
                        document.getElementById("loanstartdate").value = break_resp[16];
                        var newdate = break_resp[17].substr(0, 10);
                        newdate = newdate.split("-");
                        break_resp[17] = newdate[2] + "/" + newdate[1] + "/" + newdate[0];
                        document.getElementById("loanenddate").value = break_resp[17];
                        document.getElementById("repayoption").value = break_resp[18];
                        document.getElementById("amountperrepay").value = numberFormat(break_resp[19]);
                    }
...
//alert(readCookie('transtypes'));
                    if (readCookie('names') === 'names') {
                        ...
                        if (break_resp[12] === null || break_resp[12] === '')
                            break_resp[12] = '0';
                        document.getElementById(balance_bid).value = numberFormat(break_resp[13]);
...
//return true;
        ...
        if (resp.match("postedTransactions")) {
